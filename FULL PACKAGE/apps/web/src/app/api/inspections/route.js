  ...
  try {
    ...
    let query = `
      SELECT 
        ...
        ST_X(i.gps_location) as longitude, ST_Y(i.gps_location) as latitude,
  ...
  try {
    ...
    const result = await sql`
      INSERT INTO inspections (
        inspection_id, asset_id, inspector_id, condition_rating,
        findings, recommendations, photos, gps_location,
        weather_conditions, inspection_type
      ) VALUES (
        ${inspection_id}, ${asset_id}, ${inspector_id}, ${condition_rating},
        ${findings}, ${recommendations}, ${photos || []},
        ${longitude && latitude ? sql`ST_SetSRID(ST_MakePoint(${longitude}, ${latitude}), 4326)` : null},
