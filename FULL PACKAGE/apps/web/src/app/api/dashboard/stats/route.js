...
export async function GET(request) {
  try {
    ...
    const totalPendingApprovals = 
      parseInt(pendingApprovals[0].pending_assets) + 
      parseInt(pendingApprovals[0].pending_inspections) + 
      parseInt(pendingApprovals[0].pending_work_orders);

    return Response.json({
      assets: {
        total: totalAssets,
        by_type: assetStats,
        critical_count: parseInt(criticalAssets[0].critical_assets)
      },
      inspections: {
        ...inspectionStats[0],
        total_inspections: parseInt(inspectionStats[0].total_inspections),
        pending_inspections: parseInt(inspectionStats[0].pending_inspections),
        approved_inspections: parseInt(inspectionStats[0].approved_inspections),
        recent_inspections: parseInt(inspectionStats[0].recent_inspections)
      },
      work_orders: {
        ...workOrderStats[0],
        total_work_orders: parseInt(workOrderStats[0].total_work_orders),
        pending_work_orders: parseInt(workOrderStats[0].pending_work_orders),
        in_progress_work_orders: parseInt(workOrderStats[0].in_progress_work_orders),
        completed_work_orders: parseInt(workOrderStats[0].completed_work_orders),
      ...
      pending_approvals: {
        ...
        assets: parseInt(pendingApprovals[0].pending_assets),
        inspections: parseInt(pendingApprovals[0].pending_inspections),
        work_orders: parseInt(pendingApprovals[0].pending_work_orders)
      },
